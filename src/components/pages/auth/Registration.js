...
const Registration = () => {
  ...
  const[registerUser,{isLoading, isError}] = useRegisterUserMutation(); //using redux
  ...
  const handleSubmit =  async(e) => {
    ...
    if (actualData.name && actualData.email && actualData.password && actualData.password_confirmation && actualData.tc !== null) {
      if (actualData.password === actualData.password_confirmation) {
        ...
        const res = await registerUser(actualData);   //using redux
        console.log(res)
        if(res.data.status==="success"){    //backend Check Error
